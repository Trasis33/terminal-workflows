...
<feature>
  ...
  <behavior>
    ...
    WorkflowSource.String(i) → "name description tag1 tag2" (concatenated searchable text)
    WorkflowSource.Len() → len(workflows)
  </behavior>
  <implementation>
    1. Run `go get` for all Phase 2 dependencies first (see task 1)
    2. RED: Write tests for ParseQuery (4 cases), Search (5 cases), WorkflowSource (2 cases)
    ...
    - WorkflowSource: implements sahilm/fuzzy Source interface (String(i) string, Len() int). String concatenates Name + " " + Description + " " + strings.Join(Tags, " ").
...
<task type="auto">
  ...
  <action>
    ...
    **RED:** Create `internal/picker/search_test.go` with tests:
    ...
    7. TestWorkflowSource — verify String(i) concatenates fields, Len() returns count
  ...
  <done>ParseQuery correctly splits @tag prefix. Search returns fuzzy matches filtered by tag. Empty query returns all. WorkflowSource implements sahilm/fuzzy Source interface. All 7+ test cases pass.</done>
</task>
