...
**IMPORTANT:** Use `CONOUT$` (NOT `CONIN$`). The 06-RESEARCH.md Pattern 3 example (line ~166) shows `CONIN$` for a different purpose (input reading). Our use case is output rendering for the TUI, which requires `CONOUT$`. Do not confuse them.
...
go run ./cmd/wf completion powershell  # Verify Cobra auto-generates PowerShell completions
go test ./...
```

Note: `wf completion powershell` is auto-provided by Cobra's built-in completion command. No explicit plan is needed to implement it — just verify it outputs valid PowerShell completion script.
</verification>
