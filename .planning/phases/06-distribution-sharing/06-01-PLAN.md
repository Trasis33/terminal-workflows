...
must_haves:
  truths:
    - "Cloned remote sources are stored in the XDG data home directory (wf/sources)"
    - "User can add a remote source by URL and it clones successfully"
    - "User can remove a source and its clone directory is cleaned up"
    - "User can update a source and see a summary of what changed"
    - "User can list all configured sources"
    - "Clear error message when git is not installed (not a crash or hang)"
    - "Git clone does not hang on auth prompts (silent failure instead)"
  artifacts:
