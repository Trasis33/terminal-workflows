...
<task type="auto">
  ...
  <action>
Scope note: This task touches 4 files but each change is a mechanical one-liner (getStore→getMultiStore swap), plus the getMultiStore helper in root.go. Execute in order: (a) root.go setup first, then (b) the three store swaps.
...
3. **cmd/wf/manage.go:**
    ...
    - RESOLVED: `manage.Run(s store.Store)` already accepts the `store.Store` interface — no signature change needed. This is a one-line swap.
...
4. **cmd/wf/list.go:**
    ...
    - Note: list.go only calls `List()` on the store, never `Get()`. MultiStore.List() is the only method exercised here.

Data flow note: The picker in pick.go loads workflows via `s.List()` into a `[]Workflow` slice, then passes that slice to `picker.New(workflows)`. The picker does NOT call `Get()` after initial load — it operates entirely on the pre-loaded slice. This means MultiStore only needs a working `List()` for the picker path.
  </action>
  <verify>
