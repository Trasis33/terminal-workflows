...
**Edge case — HISTSIZE=0:** If the user has set `HISTSIZE=0` (history disabled), `history 1` returns nothing and `$_last` will be empty. The `[[ -n "$_last" ]]` guard prevents writing a blank sidecar in this case; `wf register` will fall through to the `$HISTFILE` fallback and emit its warning. No special handling is needed beyond the existing guard.
...
Note: `printf '%s'` is used instead of `echo` to avoid fish's `echo` interpreting escape sequences (e.g. a command containing `\n`) and to avoid a trailing newline being added inconsistently across fish versions.
