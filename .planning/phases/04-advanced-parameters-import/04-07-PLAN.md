...
[[snippets]]
  ...
  output      = ""
`
    result, err := ImportPet([]byte(input))
    if err != nil {
...
Add `"reflect"` to the import block if it is not already imported.

Do not modify any existing tests.
  </action>
  <verify>go test ./internal/importer/... -run "TestPetImport_BareStringTag|TestPetImport_ArrayTag" -v</verify>
